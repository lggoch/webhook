...
def getTheJson(req):
    if req.get("result").get("action") != "lastcall":
        return{}
    result = req.get("result")
    parameters = result.get("parameters")
    date = parameters.get("date")
    fname = parameters.get("firstname")
    lname = parameters.get("lastname")
    time = parameters.get("time")
    email = parameters.get("email")
    duration_n = parameters.get("duration").get("amount")
    duration_u = parameters.get("duration").get("unit")
    dat = date + "," + fname + "," + lname + "," + time + "," + email + "," + duration_n + "," + duration_u
    fileWrite(dat)


def fileWrite(dat : string) :
    file = open("json_txt.txt")
    file.write(dat)
